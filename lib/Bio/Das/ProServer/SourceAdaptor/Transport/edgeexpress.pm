...
sub init {
  ...
  my $eeDB = EEDB::Database->new_from_url($config->{eedb_url});
  if($eeDB) {
    printf("EEDB::Database = %s\n", $eeDB->url); 
  } else {
...
=head2 database
  ...
  Returntype : EEDB::Database object connected to EdgeExpressDB
  Exceptions : none
