...
def fetchmiRNAData(lib1, lib3, lib6, mature):
	...
	if mature:
		cursor.execute("select fl1.mat_mirna_id fid,mat_miRNA as primary_name,library_name,e.norm_express "
		+"from experiment join feature_expression e "
		+"using(experiment_id) join "
		+"(select mat.primary_name mat_miRNA,feature2_id mat_mirna_id,feature1_id mirna_probe_id "
		+"from feature probe join feature_link on(probe.feature_id = feature1_id) "
		+"join feature mat on(mat.feature_id = feature2_id) where link_source_id=43 "
		+"and feature2_id in ("+idstring+"))fl1 where feature_id = mirna_probe_id "
		+"and library_name in ("+datasetstring+")")
	else:
		cursor.execute("select substring(fl3.mat_miRNA,length(fl3.mat_miRNA)-1) mat_miRNA,pre_mirna_id fid, pre_miRNA primary_name,library_name,norm_express "
		+"from experiment join feature_expression e "
		+"using(experiment_id) join (select fl1.*, fl2.feature1_id mirna_probe_id "
		+"from feature_link fl2 join "
		+"(select pre.primary_name pre_miRNA, mat.primary_name mat_miRNA, feature1_id pre_mirna_id, feature2_id mat_mirna_id "
		+"from feature pre join feature_link on(pre.feature_id = feature1_id) "
		+"join feature mat on(mat.feature_id = feature2_id) "
		+"where link_source_id=20 and feature1_id in ("+idstring+"))fl1 "
...
def fetchCAGEData(lib1, lib3, lib6):
    cursor.execute("select feature_id fid, primary_name, library_name, norm_express "
    +"from experiment "
    +"join (select * from feature_expression "
    +"join feature "
    +"using(feature_id) "
    +"where feature.feature_id in ("+idstring+"))e "
    +"using(experiment_id) "
    +"where platform='CAGE' and library_name in ("+datasetstring+") "
    +"order by feature_id, library_name, time_point_hr;")

    parseResultSet(lib1, lib3, lib6)
    
def fetchQRTData(lib1, lib3, lib6):
	cursor.execute("SELECT enz.feature_id fid, enz.primary_name primary_name, "
	+"library_name, norm_express, f2.primary_name primer_name, time_point_hr "
	+"FROM feature enz "
	+"JOIN feature_link fl1 "
	+"on(fl1.feature2_id=enz.feature_id) JOIN feature f2 "
	+"on(fl1.feature1_id=f2.feature_id) JOIN feature_expression fe "
	+"on(f2.feature_id = fe.feature_id) JOIN experiment using(experiment_id ) "
	+"WHERE fl1.link_source_id=25 and enz.feature_id in ("+idstring+") "
	+"and library_name in ("+datasetstring+") "
	+"UNION SELECT f2.feature_id fid, f2.primary_name primary_name, library_name, "
	+"norm_express, f2.primary_name primer_name, time_point_hr  FROM feature f2  "
	+"JOIN feature_expression fe  on(f2.feature_id = fe.feature_id) JOIN experiment using(experiment_id )  "
	+"WHERE platform='qRT_PCR' and f2.feature_id in ("+idstring+") "
	+"order by fid, primer_name, library_name, time_point_hr")
...
def fetchIlluminaData(lib1, lib3, lib6):
    cursor.execute("SELECT f2.primary_name id "
        +"FROM feature enz "
        +"JOIN feature_link fl1 on(fl1.feature2_id=enz.feature_id) "
        +"JOIN feature f2 on( fl1.feature1_id=f2.feature_id) "
        +"JOIN feature_expression fe on(f2.feature_id = fe.feature_id) "
        +"WHERE fl1.link_source_id=28 and enz.feature_id in ("+idstring+") "
        +"and detection_score>0.99 "
    ...
    if len(probeIds)>0:
        cursor.execute("SELECT enz.feature_id fid,enz.primary_name, "
            +"f2.primary_name ilm_id, library_name, norm_express "
            +"FROM feature enz JOIN feature_link fl1 "
            +"on(fl1.feature2_id=enz.feature_id ) "
            +"JOIN feature f2 on( fl1.feature1_id=f2.feature_id) "
            +"JOIN feature_expression fe on(f2.feature_id = fe.feature_id) "
            +"JOIN experiment using(experiment_id ) "
            +"WHERE fl1.link_source_id=28 "
            +"and library_name in ("+datasetstring+") "
            +"and enz.feature_id in ("+idstring+") "
            +"and f2.primary_name in("+probeIds+") "
            +"and experiment.experiment_id not in(71,72) "
            +"order by fid,ilm_id,library_name,time_point_hr")
...
def parseResultSet(lib1, lib3, lib6):
    ...
    for row in rs:
        lib = {}
        probeid=''
        if row['library_name'].find("RIKEN1")==0:
            lib = lib1
        elif row['library_name'].find("RIKEN3")==0:
            lib = lib3
        else:
            lib = lib6
...
conn = MySQLdb.connect(host="fantom43.gsc.riken.jp",user="read",passwd="read",db="eeDB_fantom4_may08")
