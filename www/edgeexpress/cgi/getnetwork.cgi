...
BEGIN{
    ...
    unshift(@INC, "/usr/local/edgeexpress/EdgeExpress_1.004/modules");
}
...
$eeDB = EEDB::Database->new(
				'-host'=>'fantom40.gsc.riken.jp', 
				'-port'=>'3306', 
				'-user'=>'read', 
				'-password'=>'read', 
				'-database'=>'eeDB_fantom4'); 
...
sub test_edge_filters {
  ...
  my $type = '';
  if($edge->edge_source->name eq 'Entrez_TFmatrix_L2_L3_Entrez_may2008') { $type = 'tfbs'; }
  if($edge->edge_source->name eq 'Entrez_TFmatrix_L2_miRNA') { $type = 'tfbs'; }
  if($edge->edge_source->name eq 'ChIP_chip') { $type = 'chip'; }
  if($edge->edge_source->name eq 'siRNA_perturbation') { $type = 'perturb'; }
  if($edge->edge_source->name eq 'pre-miRNA_perturbation') {$type = 'perturb'; }
  if($edge->edge_source->name eq 'miRNA_targets') { $type = 'mirnaT'; }

  if($edge->edge_source->name =~ /PPI/) { $type = 'ppi'; }
  elsif($edge->edge_source->classification eq 'Published') { $type = 'pub'; }
...
sub get_node_express_info {
  ...
  foreach my $edge (@{$edges}) {
    ...
    my $express_array = EEDB::Expression->fetch_all_by_feature($edge->feature1, "norm");
    my $minexp=undef;
