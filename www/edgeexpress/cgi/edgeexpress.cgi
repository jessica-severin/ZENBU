...
sub get_peers {
  ...
  foreach my $peer (values(%peers)) {
    cache_peer_sources($self, $peer);
  }

  foreach my $source (values(%{$global_source_cache})) {
    unless($peers{$source->peer_uuid}) { next; }

    if($self->{'apply_source_filter'}) {
      if($self->{'filter_ids'}->{$source->db_id}) {
        $self->{'known_sources'}->{$source->db_id} = $source;
      }
      if(($source->class eq "FeatureSource") and $self->{'filter_sourcenames'}->{$source->name}) {
        $self->{'known_sources'}->{$source->db_id} = $source;
      }
    } else {
      $self->{'known_sources'}->{$source->db_id} = $source;
    }
    if(defined($self->{'expfilter'}) and (($source->class eq "Experiment") or ($source->class eq "FeatureSource"))) {
      if($source->metadataset->check_by_filter_logic($self->{'expfilter'})) {
        $self->{'known_sources'}->{$source->db_id} = $source;
      }
    }

    if(defined($self->{'known_sources'}->{$source->db_id}) and
      (($source->class eq "Experiment") or ($source->class eq "FeatureSource"))) {
      my $dtypes = $source->expression_datatypes;
      foreach my $type (@$dtypes) {
        $self->{'known_sources'}->{$type->db_id} = $type;
      }
    }
  }
  my @ps = values(%peers);
...
sub find_peer {
  ...
  unless($peer) {
    ...
    $peer = $start_peer->find_peer($uuid, 2);
  }
  return $peer;
}


sub cache_peer_sources {
  my $self = shift;
  my $peer = shift;

  unless($peer) { return; }
  unless($peer->test_is_valid) { return; }
  if(!($peer->{'_register_sources'})) {
    ...
    while (my $source = $stream->next_in_stream) {
      ...
      $global_source_cache->{$source->db_id} = $source;
    }
    $peer->{'_register_sources'} = 1;
    $stream->disconnect;
  }
}
...
sub get_center_feature {
  ...
  return $stream->fetch_object_by_id($self->{'id'});
  ...
  #  }
  #}
  #return $db->fetch_object_by_id($fid);
}
